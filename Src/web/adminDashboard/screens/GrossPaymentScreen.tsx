...
const GrossPaymentScreen = () => {
  ...
  const [globalPayFrequency, setGlobalPayFrequency] = useState<GrossPayment['payFrequency']>(DEFAULT_PAY_FREQUENCY);
  ...
  const handleInputChange = (key: keyof GrossPayment, value: string | number) => {
    setFormData(prev => prev ? { ...prev, [key]: value } : null);
  };

  const handleSave = () => {
    if (!formData) return;
    const updatedGrossPay = (formData.hoursWorked * formData.hourlyRate + (formData.bonuses || 0));
  ...
  const getStatusBadgeStyle = (status: GrossPayment['status']) => {
    switch (status) {
      case 'Approved': return { backgroundColor: '#27ae60', color: 'white', padding: 4, borderRadius: 4 };
      case 'Paid': return { backgroundColor: '#2980b9', color: 'white', padding: 4, borderRadius: 4 };
      default: return { backgroundColor: '#f39c12', color: 'white', padding: 4, borderRadius: 4 };
    }
  };
      ...
      <View style={[styles.mainContent, { marginLeft: isSidebarOpen ? 250 : 0 }]}>
        ...
        <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 10 }}>
          ...
          <Picker
            ...
            onValueChange={value => setGlobalPayFrequency(value as GrossPayment['payFrequency'])}
            style={{ flex: 1 }}>
        ...
        <ScrollView horizontal>
          <View style={styles.table}>
            <View style={styles.tableHeader}>
              {['ID', 'Name', 'Pay Period', 'Freq', 'Hours', 'Rate', 'Bonus', 'Tax', 'Gross', 'Net', 'Status', 'Actions'].map(h => (
            ...
            {filteredPayments.map(p => (
              <View key={p.id} style={styles.row}>
                ...
                <Text style={styles.cell}>{`${p.payPeriodStart} - ${p.payPeriodEnd}`}</Text>
                ...
                <Text style={[styles.cell, getStatusBadgeStyle(p.status)]}>{p.status}</Text>
                <View style={styles.cell}>
                  <TouchableOpacity onPress={() => handleEdit(p)}><Text>Edit</Text></TouchableOpacity>
                  <TouchableOpacity onPress={() => handleDelete(p.id)}><Text style={{ color: 'red' }}>Delete</Text></TouchableOpacity>
                </View>
              </View>
            ))}
          </View>
        ...
        <Text style={{ marginTop: 10, fontStyle: 'italic', fontSize: 12, color: 'gray' }}>
          *Note: Tax calculations are approximate and for demonstration purposes only.
        </Text>
...
<Modal visible={modalVisible} animationType="slide" transparent>
          <View style={{ flex: 1, backgroundColor: '#00000099', justifyContent: 'center', padding: 20 }}>
            <View style={{ backgroundColor: 'white', padding: 20, borderRadius: 10 }}>
              <Text style={{ fontWeight: 'bold', fontSize: 18, marginBottom: 10 }}>Edit Hourly Rate</Text>
              {formData && (
                <>
                  <Text style={styles.inputLabel}>Employee: {formData.employeeName}</Text>
                  <Text style={styles.inputLabel}>Hours Worked: {formData.hoursWorked}</Text>
                  <Text style={styles.inputLabel}>Bonuses: ${formData.bonuses ?? 0}</Text>
                  <Text style={styles.inputLabel}>Tax Deduction: ${formData.deductions ?? 0}</Text>
                  <TextInput
                    style={styles.input}
                  ...
                  <Picker
                    ...
                    onValueChange={(value) => handleInputChange('status', value as GrossPayment['status'])}>
                  ...
                  <View style={{ flexDirection: 'row', justifyContent: 'flex-end', marginTop: 10 }}>
                    <TouchableOpacity onPress={() => setModalVisible(false)} style={[styles.addButton, { backgroundColor: '#aaa', marginRight: 10 }]}> 
