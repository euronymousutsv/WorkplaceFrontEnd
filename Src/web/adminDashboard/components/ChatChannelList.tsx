...
import { addAccessToChannel, createNewChannel, getAllChannelForCurrentServer } from '../../../api/server/channelApi';
...
const ChatChannelList: React.FC = () => {
  ...
  const [highestRoleToAccessChannel, setHighestRoleToAccessChannel] = useState<'employee' | 'manager' | 'admin'>('employee');
    ...
    try {
      ...
      if (res instanceof ApiError) {
        ...
        const parsedChannels = res.data.map((channel) => ({
          ...
          highestRoleToAccessChannel: channel.highestRoleToAccessChannel as 'admin' | 'manager' | 'employee',
        }));
...
<View style={styles.roleSelectContainer}>
  ...
  {['employee', 'manager', 'admin'].map((role) => (
    <TouchableOpacity
      ...
      style={[
        styles.roleOption,
        highestRoleToAccessChannel === role && styles.roleOptionActive,
      ]}
