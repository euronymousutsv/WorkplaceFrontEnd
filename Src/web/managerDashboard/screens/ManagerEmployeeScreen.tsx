...
if (ids.length === 0) {
  ...
  const handleSave = async () => {
    ...
    if (editingEmployee) {
      if (
        !formData.role ||
        !roleMapper[formData.role as keyof typeof roleMapper]
      ) {
        Toast.show({
          type: "error",
          text1: "Please select a valid role",
        });
        return;
      }
      ...
      if (
        !formData.status ||
        !statusMapper[formData.status as keyof typeof statusMapper]
      ) {
        Toast.show({
          type: "error",
      ...
      try {
        const payload = {
          ...
          role: roleMapper[formData.role as keyof typeof roleMapper],
          employmentStatus: statusMapper[formData.status as keyof typeof statusMapper],
        };
